...
export class AppComponent implements OnInit {
  ...
  prepareAdd(): void {
    this.isEdit = false;
    this.cus = { id: 0, customerCode: '', fullName: '', age: 18, address: '' };
    this.showModal();
  }
  ...
  handleOk(): void {
    ...
    //   });
    // }
    if (this.cus.customerCode || this.cus.address || this.cus.age || this.cus.fullName) {
      if (this.isEdit) {
        this.customerService.updateCustomer(this.currentEditId!, this.cus).subscribe(() => {
          this.message.success('Cập nhật thông tin khách hàng thành công !');
        });
      } else {
        this.customerService.checkDuplicateCode(this.cus.customerCode).subscribe((result: Customer[]) => {
          if (result && result.length > 0) {
            this.message.error("Mã khách hàng này đã tồn tại trong hệ thống !");
          } else {
